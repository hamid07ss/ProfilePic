...
// JSON.stringify([].slice.call($$('.named-color-table tr'), 1).map(function(row) { return [row.childNodes[3].textContent, row.childNodes[5].textContent.trim().split(",").map(Number)] }).reduce(function(data, row) {data[row[0]] = row[1]; return data}, {}))
    ...
    NodeParser.prototype.paintText = function (container) {
        ...
        var textList = getWords(characters);
            ...
            /*(!this.options.letterRendering || noLetterSpacing(container)) && !hasUnicode(container.node.data) ? getWords(characters) : characters.map(function (character) {
    ...
    function getWords(characters) {
        ...
        while (characters.length) {
            if (isWordBoundary(characters[i]) === onWordBoundary) {
                ...
                if (word.length) {
                    var hamid = window.html2canvas.punycode.ucs2.encode(word);
                    if(check(hamid)){
                        var ch = check(hamid);
                        hamid = hamid.replace(ch, '');
                        words.push(hamid);
                        words.push(ch);
                    }else{
                        words.push(hamid);
    ...
    function check(s) {
        ...
        if(t){
            s = s.trim();
            if (jQuery.inArray(s[s.length-1], ch) != -1){
                return ch[jQuery.inArray(s[s.length-1], ch)];
            }
        }
